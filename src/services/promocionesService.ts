...
export const promocionesService = {
  async list(): Promise<Promocion[]> {
    ...
    return (data || []).map(p => ({
      ...
      dias_vigentes: (p.vigencia !== null && p.vigencia !== undefined && p.vigencia !== '') ? p.vigencia : 'Todos los días',
  ...
  async create(promo: Promocion): Promise<Promocion> {
    ...
    try {
      ...
      return {
        ...
        dias_vigentes: (data.vigencia !== null && data.vigencia !== undefined && data.vigencia !== '') ? data.vigencia : 'Todos los días',
        activo: data.activa,
